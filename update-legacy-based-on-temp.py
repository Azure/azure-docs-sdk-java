...
TARGET_SOURCE_FOLDER = "temporary-src-based-yml"
...
if __name__ == "__main__":
  ...
  for index, top_level_toc_item in enumerate(legacy_toc):
    if check_against_targeted_namespaces(top_level_toc_item['uid'], migrating_namespaces_regexs):
      ...
      legacy_toc_items.append(top_level_toc_item)

  appended_content = yaml.dump(toc_items, default_flow_style=False)
  appended_legacy_content = yaml.dump(legacy_toc_items, default_flow_style=False)
  ...
  with open(os.path.join(root_dir, TARGET_SOURCE_FOLDER, "toc.yml"), "w", encoding="utf-8") as stable_toc:
    stable_toc.write(appended_legacy_content)
